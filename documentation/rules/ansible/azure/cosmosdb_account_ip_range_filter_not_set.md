...
Cosmos DB accounts should have an IP range filter configured to restrict which client IP addresses can connect, because without it the account may accept connections from unintended networks and increase the risk of unauthorized data access. In Ansible, the `azure.azcollection.azure_rm_cosmosdbaccount` (and legacy `azure_rm_cosmosdbaccount`) resource must include the `ip_range_filter` property set to the allowed IP addresses or CIDR ranges. Resources missing `ip_range_filter` or with it empty will be flagged as they indicate no network-level IP restrictions; provide a comma-separated list of IPs/CIDRs to enforce access control.
