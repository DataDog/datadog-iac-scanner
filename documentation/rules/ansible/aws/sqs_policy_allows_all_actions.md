...
SQS queue policies must not grant wildcard ('*') actions because allowing all actions to a principal enables unauthorized access, message retrieval or deletion, and queue modification, which can lead to data exposure or service disruption. For Ansible SQS resources (`community.aws.sqs_queue` and `sqs_queue`), inspect the `policy` document and ensure no Statement with `Effect: "Allow"` has `Action` equal to `*` or contains `*`. Resources with `Action='*'` or `Action` arrays that include `*` will be flagged; instead, specify explicit SQS actions (for example `sqs:SendMessage`, `sqs:ReceiveMessage`, `sqs:DeleteMessage`) and restrict principals to the minimum required.
