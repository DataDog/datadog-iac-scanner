...
const (
	yamlKeyConfigMapGenerator = "configMapGenerator:"
	yamlKeySecretGenerator    = "secretGenerator:"
)

func isGeneratorSectionHeader(trimmed string) bool {
	return trimmed == yamlKeyConfigMapGenerator || trimmed == yamlKeySecretGenerator
}

// generatorConfigLine returns the best 1-based line for a generator-produced resource in the kustomization file.
func generatorConfigLine(origin *model.KustomizeOrigin, resourceName string) (line1 int, path string) {
	...
	if ln, pth, ok := generatorLineFromScan(lines, resourceName, p); ok {
		return ln, pth
	}
	return generatorLineFallback(lines, resourceName, p)
}

func generatorLineFromScan(lines []string, resourceName, p string) (line1 int, path string, ok bool) {
	inGen := false
	for i, line := range lines {
		trimmed := strings.TrimSpace(line)
		if !inGen {
			if isGeneratorSectionHeader(trimmed) {
				inGen = true
			}
			continue
		}
...
func generatorLineFallback(lines []string, resourceName, p string) (line1 int, path string) {
	for i, line := range lines {
		for _, candidate := range generatorNameCandidates(resourceName) {
...
func transformerConfigLine(origin *model.KustomizeOrigin) (line1 int, path string) {
...
func transformerPatchReferenceLine(origin *model.KustomizeOrigin) (line1 int, path string) {
	...
	for _, tr := range origin.Transformations {
		...
		data, err := rootfile.ReadFile(kustPath)
		if err != nil {
			continue
		}
		lines := strings.Split(string(data), "\n")
...
func transformerLineForOrigin(origin *model.KustomizeOrigin) (line1 int, path string) {
	if origin == nil {
		return 1, ""
	}
...
func appendPatchPathsMixedList(list []interface{}, add func(string)) {
...
func patchPathsDeclaredInKustomization(kustPath string) []string {
	clean := filepath.Clean(kustPath)
	data, err := rootfile.ReadFile(clean)
	if err != nil {
		return nil
	}
	...
	if list, ok := doc["patches"].([]interface{}); ok {
		appendPatchPathsMixedList(list, add)
	}
