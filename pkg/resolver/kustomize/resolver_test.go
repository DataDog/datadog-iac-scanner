...
`))
	...
	require.Len(t, out.Diagnostics, 1)
	require.Equal(t, "kustomize-remote-disallowed", out.Diagnostics[0].QueryID)
	require.Contains(t, out.Diagnostics[0].Message, "github.com/Liujingfang1/mysql?ref=test")
}
...
`))
	...
	var sawInflationDisabled bool
	for _, d := range out.Diagnostics {
		require.NotEqual(t, "kustomize-remote-disallowed", d.QueryID, "%+v", d)
		if d.QueryID == "kustomize-helm-inflation-disabled" {
			sawInflationDisabled = true
		}
	}
	require.True(t, sawInflationDisabled, "%+v", out.Diagnostics)
