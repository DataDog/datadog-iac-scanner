...
func PrepareHelmChartsIfNeeded(
...
) (buildRoot string, fsRoot string, diags []model.ResolverDiagnostic, _ error) {
	repoRoot = filepath.Clean(repoRoot)
	kustomRoot = filepath.Clean(kustomRoot)
	kf := kustomizationEntryFile(kustomRoot)
	data, err := os.ReadFile(filepath.Join(kustomRoot, kf))
	if err != nil {
	...
	if len(hc) == 0 {
		// Recurse: a child base may declare helmCharts even when this root does not.
		var childHasHelm bool
		_ = WalkLocalKustomizations(kustomRoot, func(kustPath string, raw []byte) error {
			...
			if hc, _ := doc["helmCharts"].([]interface{}); len(hc) > 0 {
				childHasHelm = true
			}
			return nil
		})
	...
	_ = os.MkdirAll(filepath.Dir(repoConfig), 0o700)
	_ = os.MkdirAll(repoCache, 0o700)
	_ = os.MkdirAll(filepath.Dir(registryConfig), 0o700)

	if err := WalkLocalKustomizations(staging, func(kustPath string, raw []byte) error {
		var err error
		diags, err = rewriteHelmChartsInKustomization(ctx, kustPath, raw, stagedRepo, scratchDir, helmIncludeCRDs, allowInflation, repoConfig, repoCache, registryConfig, diags)
		return err
	}); err != nil {
...
func rewriteHelmChartsInKustomization(
	...
	if !allowInflation {
		...
		if err := os.WriteFile(kustPath, outK, 0o600); err != nil {
			return diags, err
		}
	...
	for i, item := range hc {
		...
		chartPath := filepath.Join(kustDir, chartHome, name)
		if strings.TrimSpace(repo) == "" {
			if _, err := os.Stat(chartPath); err != nil {
				contextLogger.Warn().Msgf("kustomize helm chart not found at %s", chartPath)
				diags = append(diags, model.ResolverDiagnostic{
					FilePath: kustPath,
					Message:  "helm chart not found: " + chartPath,
			...
			// Remote chart: Helm resolves by ref, not a local chartPath.
			chartPath = name
		}
		...
		if err := os.WriteFile(outPath, joinYAML(blobs), 0o600); err != nil {
			diags = append(diags, model.ResolverDiagnostic{
				FilePath: outPath,
				Message:  err.Error(),
