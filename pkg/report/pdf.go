...
func createHeaderArea(m pdf.Maroto) {
	...
	m.Row(rowXLarge, func() {
		m.Col(colSix, func() {
			m.Text(" DATADOG IAC SCANNER REPORT", props.Text{
				Size:        25,
				Style:       consts.Bold,
				Align:       consts.Left,
				Extrapolate: false,
...
func PrintPdfReport(ctx context.Context, path, filename string, body interface{}, sciInfo *model.SCIInfo) error {
	...
	startTime := time.Now()
	contextLogger.Info().Msg("Started generating pdf report")
	...
	contextLogger.Info().Msgf("Generate report duration: %v", time.Since(startTime))
