...
func (info *k8sAPICall) saveK8sResources(ctx context.Context, kind, k8sResourcesContent, apiVersionFolder string) {
	...
	if err != nil {
		contextLogger.Error().Msgf("failed to open file '%s': %s", file, err)
	}

	if _, err = f.WriteString(k8sResourcesContent); err != nil {
		contextLogger.Error().Msgf("failed to write file '%s': %s", file, err)
	}

	err = f.Close()
	if err != nil {
		contextLogger.Err(err).Msgf("failed to close file: %s", file)
	}
}
