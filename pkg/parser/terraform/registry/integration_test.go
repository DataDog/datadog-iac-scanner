...
`
	...
	for _, tt := range tests {
		t.Run(tt.address, func(t *testing.T) {
			location, found := reg.LookupWithScope(tt.address, testFile)
			if found != tt.shouldExist {
				t.Errorf("LookupWithScope(%q) found=%v, want %v", tt.address, found, tt.shouldExist)
			}
			if found && tt.expectedLine > 0 && location.Line != tt.expectedLine {
				t.Errorf("LookupWithScope(%q) line=%d, want %d", tt.address, location.Line, tt.expectedLine)
			}
			if found && location.FilePath != testFile {
				t.Errorf("LookupWithScope(%q) file=%s, want %s", tt.address, location.FilePath, testFile)
			}
		})
	}
	...
	for _, tt := range indexedTests {
		t.Run("indexed_"+tt.address, func(t *testing.T) {
			...
			if found {
				...
				baseLocation, _ := reg.LookupWithScope(tt.expectedBase, testFile)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if moduleAddr != "" {
				...
				if len(parts) >= 2 {
					...
					location, found = reg.LookupWithScope(topLevelModule, mainFile)
				}
				if !found {
					location, found = reg.LookupWithScope(moduleAddr, mainFile)
				}
			} else {
				location, found = reg.LookupWithScope(tt.tfplanAddr, mainFile)
			}
