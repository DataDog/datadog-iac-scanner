...
// ShellScriptParser parses shell scripts using tree-sitter
type ShellScriptParser struct {
	bashLang *sitter.Language
}

// NewShellScriptParser creates a new shell script parser
func NewShellScriptParser() *ShellScriptParser {
	return &ShellScriptParser{
		bashLang: sitter.NewLanguage(bash.Language()),
	}
}

// ParseRunBlock parses a shell script and returns structured command information
func (p *ShellScriptParser) ParseRunBlock(ctx context.Context, runScript, shell string) *ParsedRun {
	result := &ParsedRun{
		Shell:    shell,
	...
	result.Commands = p.extractCommands(tree.RootNode(), scriptBytes)
	result.ParseOK = true
...
func (p *ShellScriptParser) extractCommands(node *sitter.Node, source []byte) []ParsedCommand {
	commands := []ParsedCommand{}

	cursor := node.Walk()
	defer cursor.Close()

	p.walkNode(cursor, source, &commands)

	return commands
}

// walkNode recursively walks the AST node by node
func (p *ShellScriptParser) walkNode(cursor *sitter.TreeCursor, source []byte, commands *[]ParsedCommand) {
	node := cursor.Node()
	...
	case command:
		...
		cmd := p.parseCommand(node, source)
		if cmd != nil {
	...
	case redirected_statement:
		...
		cmd := p.parseRedirectedStatement(node, source)
		if cmd != nil {
			*commands = append(*commands, *cmd)
		}
	...
	case pipeline:
		...
		cmd := p.parsePipeline(node, source)
		if cmd != nil {
			*commands = append(*commands, *cmd)
		}
		// Don't recurse - we've already manually parsed the children
...
func (p *ShellScriptParser) parseCommand(node *sitter.Node, source []byte) *ParsedCommand {
	cmd := &ParsedCommand{
		Type: command,
		Args: []ParsedArg{},
	}
...
func (p *ShellScriptParser) parseRedirectedStatement(node *sitter.Node, source []byte) *ParsedCommand {
	cmd := &ParsedCommand{
	...
	for i := uint(0); i < childCount; i++ {
		...
		switch child.Kind() {
		case command:
			innerCmd := p.parseCommand(child, source)
			if innerCmd != nil {
		...
		case file_redirect:
			cmd.Redirect = p.parseRedirect(child, source)
		}
	}
...
func (p *ShellScriptParser) parseRedirect(node *sitter.Node, source []byte) *ParsedRedirect {
	...
	for i := uint(0); i < childCount; i++ {
		...
		if slices.Contains(redirectOperators, childKind) {
			...
			redirect.Target = p.parseArg(child, source)
		}
	}
...
func (p *ShellScriptParser) parsePipeline(node *sitter.Node, source []byte) *ParsedCommand {
	cmd := &ParsedCommand{
		Type:     pipeline,
...
func (p *ShellScriptParser) parseArg(node *sitter.Node, source []byte) ParsedArg {
	arg := ParsedArg{
		Type:  node.Kind(),
...
func (p *ShellScriptParser) hasExpansion(node *sitter.Node) bool {
	childCount := node.NamedChildCount()
	for i := uint(0); i < childCount; i++ {
		child := node.NamedChild(i)
		if child == nil {
