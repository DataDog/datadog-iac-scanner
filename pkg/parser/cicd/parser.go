...
// Parse parses cicd file and returns it as a Document
func (p *Parser) Parse(ctx context.Context, fileContent []byte, filePath string,
	...
	for {
		...
		if node.Kind == yaml.DocumentNode && len(node.Content) > 0 {
			...
			if err := doc.UnmarshalYAML(ctx, contentNode, ignore); err != nil {
				return nil, nil, nil, nil, errors.Wrap(err, "failed to unmarshal yaml")
			}
	...
	if len(documents) == 0 {
		return nil, nil, nil, nil, errors.New("no documents found in yaml file")
	}
