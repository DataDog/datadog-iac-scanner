...
// ExpressionParser parses GitHub Actions expressions using tree-sitter
type ExpressionParser struct {
	ghactionsLang *sitter.Language
}

// getExpressionParser returns the expression parser, initializing it if needed
func (p *Parser) getExpressionParser() *ExpressionParser {
	if p.expressionParser == nil {
		p.expressionParser = NewExpressionParser()
	}
	return p.expressionParser
}

// NewExpressionParser creates a new expression parser
func NewExpressionParser() *ExpressionParser {
	return &ExpressionParser{
		ghactionsLang: sitter.NewLanguage(ghactions.Language()),
	}
}

// ParseExpression parses a GitHub Actions expression and returns structured information
func (p *ExpressionParser) ParseExpression(ctx context.Context, expr ExpressionMatch) *ParsedExpression {
	...
	defer parser.Close()
	err := parser.SetLanguage(p.ghactionsLang)
	...
	if rootNode.NamedChildCount() > 0 {
		for i := uint(0); i < rootNode.NamedChildCount(); i++ {
			...
			if child != nil && !p.isConstantReducible(child) {
...
func (p *ExpressionParser) ExtractExpressionsFromString(input string) []ExpressionMatch {
...
func (p *ExpressionParser) buildAST(node *sitter.Node, source []byte) ASTNode {
	ast := ASTNode{
		Type:     node.Kind(),
	...
	for i := uint(0); i < childCount; i++ {
		...
		if child != nil {
			ast.Children = append(ast.Children, p.buildAST(child, source))
		}
	}
...
func (p *ExpressionParser) analyzeExpression(node *sitter.Node, source []byte, result *ParsedExpression) { //nolint:gocyclo
	nodeKind := node.Kind()
	...
	case call_expression, function_call:
		...
		funcName := p.getFunctionName(node, source)
			...
			for i := uint(0); i < node.NamedChildCount(); i++ {
				...
				if child != nil && child.Kind() == argument_list {
					...
					for j := uint(0); j < child.NamedChildCount(); j++ {
						...
						if arg != nil && p.isBareSecretsContext(arg, source) {
	...
	case subscript_expression, index_expression:
		...
		if objectNode != nil && indexNode != nil {
			...
			if strings.EqualFold(objectText, "secrets") && !p.isLiteralString(indexNode) {
		...
		if indexNode != nil && p.isComputedIndex(indexNode) {
	...
	for i := uint(0); i < childCount; i++ {
		child := node.NamedChild(i)
		if child != nil {
			p.analyzeExpression(child, source, result)
		}
	}
}

// getFunctionName extracts the function name from a call expression
func (p *ExpressionParser) getFunctionName(node *sitter.Node, source []byte) string {
...
func (p *ExpressionParser) isBareSecretsContext(node *sitter.Node, source []byte) bool {
...
func (p *ExpressionParser) isLiteralString(node *sitter.Node) bool {
	kind := node.Kind()
...
func (p *ExpressionParser) isComputedIndex(node *sitter.Node) bool {
	// If it's a literal string, it's not computed
	if p.isLiteralString(node) {
		return false
	}
...
func (p *ExpressionParser) isConstantReducible(node *sitter.Node) bool {
	kind := node.Kind()
	...
	if kind == binary_expression {
		...
		if leftNode != nil && rightNode != nil {
			return p.isConstantReducible(leftNode) && p.isConstantReducible(rightNode)
		}
	}
	...
	if kind == unary_expression {
		...
		if operandNode != nil {
			return p.isConstantReducible(operandNode)
		}
	}
