...
func TestRemoteResolveIdentityStableAcrossGitSpelling(t *testing.T) {
	...
	ssh := remoteResolveIdentity(&tfmodules.ParsedModule{Source: "git::ssh://git@github.com/org/repo//sub?ref=v1", Version: ""})
	require.Equal(t, withoutGit, ssh)
}
...
func TestShouldPreScanTerraformModules(t *testing.T) {
	...
	stack := filepath.Join(root, "stack")
	require.NoError(t, os.MkdirAll(stack, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(stack, "main.tf"), []byte(`module "x" { source = "../mod" }`), 0o644))

	client := &Client{ScanParams: &Parameters{NoRemoteModules: true}}
	require.False(t, client.shouldPreScanTerraformModules([]string{stack}))

	client.ScanParams.ModulesManifestPath = filepath.Join(root, "modules.json")
	require.True(t, client.shouldPreScanTerraformModules([]string{stack}))

	client.ScanParams.ModulesManifestPath = ""
	client.ScanParams.NoRemoteModules = false
	require.True(t, client.shouldPreScanTerraformModules([]string{stack}))

	tfModulesDir := filepath.Join(stack, ".terraform", "modules")
	require.NoError(t, os.MkdirAll(tfModulesDir, 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(tfModulesDir, "modules.json"), []byte(`{"Modules":[]}`), 0o644))
	client.ScanParams.NoRemoteModules = true
	require.True(t, client.shouldPreScanTerraformModules([]string{stack}))
}
