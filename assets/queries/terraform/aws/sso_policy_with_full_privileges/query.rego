...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("aws_ssoadmin_permission_set_inline_policy[%s].inline_policy.Statement[%d].Action", [name, idx]),
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_ssoadmin_permission_set_inline_policy", name, "inline_policy", "Statement", idx, "Action"], []),
	}
}
