...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("aws_api_gateway_rest_api_policy[%s].policy.Statement[%d].Action", [name, idx]),
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_api_gateway_rest_api_policy", name, "policy", "Statement", idx, "Action"], []),
	}
}
