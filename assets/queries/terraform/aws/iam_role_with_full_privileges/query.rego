...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("aws_iam_role[%s].assume_role_policy.Statement[%d].Action", [name, idx]),
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_iam_role", name, "assume_role_policy", "Statement", idx, "Action"], []),
	}
}
