...
CxPolicy[result] {
	...
	result := {
		"documentId": input.document[i].id,
		"resourceType": resourceType,
		"searchKey": sprintf("%s[%s].%s.%s[%s].security_context", [resourceType, name, specInfo.path, types[x], j]),
		"issueType": "IncorrectValue",
...
CxPolicy[result] {
	...
	result := {
		...
		"resourceType": resourceType,
		"resourceName": tf_lib.get_resource_name(resource, name),
		"searchKey": sprintf("%s[%s].%s.%s[%d].security_context.read_only_root_filesystem", [resourceType, name, specInfo.path, types[x], y]),
		"issueType": "IncorrectValue",
