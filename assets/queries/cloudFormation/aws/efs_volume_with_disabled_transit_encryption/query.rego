...
CxPolicy[result] {
    ...
    value := efs.EFSVolumeConfiguration.TransitEncryption 
	not value == "ENABLED"
    ...
    result := {
		...
		"searchKey": sprintf("Resources.%s.Properties.Volumes.Name=%s.EFSVolumeConfiguration.TransitEncryption", [key, efs.Name]),
...
CxPolicy[result] {
	...
	elem.Type == "AWS::ECS::TaskDefinition"
    efs := elem.Properties.Volumes[index]
    not common_lib.valid_key(efs.EFSVolumeConfiguration, "TransitEncryption")

    result := {
		"documentId": input.document[i].id,
		"resourceType": elem.Type,
		"resourceName": cf_lib.get_resource_name(elem, key),
		"searchKey": sprintf("Resources.%s.Properties.Volumes.Name=%s", [key, efs.Name]),
