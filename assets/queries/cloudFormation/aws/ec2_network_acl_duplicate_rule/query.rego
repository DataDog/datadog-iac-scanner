...
getTraffic(entry) = "egress" {
	cf_lib.normalize_cloudFormation_boolean(entry.Properties.Egress) == "true"
} else = "ingress" {
	cf_lib.normalize_cloudFormation_boolean(entry.Properties.Egress) == "false"
} else = "egress" {
	cf_lib.normalize_cloudFormation_boolean(entry.Properties.Ingress) == "false"
} else = "ingress" {
	cf_lib.normalize_cloudFormation_boolean(entry.Properties.Ingress) == "true"
}
