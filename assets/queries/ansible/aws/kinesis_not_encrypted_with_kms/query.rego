...
canonical := "kinesis_stream"

CxPolicy[result] {
	task := ansLib.tasks[id][t]
	variant := ansLib.get_variants(canonical)[_]
	kinesis_stream := task[variant]
	...
	result := {
		...
		"resourceName": ansLib.get_resource_name(kinesis_stream, canonical, task),
...
CxPolicy[result] {
	...
	result := {
		...
		"resourceType": canonical,
		"resourceName": ansLib.get_resource_name(kinesis_stream, canonical, task),
		"searchKey": sprintf("name={{%s}}.{{%s}}.encryption_state", [task.name, variant]),
...
CxPolicy[result] {
	...
	result := {
		...
		"resourceType": canonical,
		"resourceName": ansLib.get_resource_name(kinesis_stream, canonical, task),
		"searchKey": sprintf("name={{%s}}.{{%s}}.encryption_type", [task.name, variant]),
		"issueType": "IncorrectValue",
