...
The purpose of this docs is to describe the E2E test suite
...
```bash
cd repository_folder/.github/scripts/server-mock
npm install
npm start
```
...
Build
```bash
LINUX/MAC: go build -tags dev -o ./bin/datadog-iac-scanner cmd/scanner
WINDOWS: go build -tags dev -o ./bin/datadog-iac-scanner.exe cmd/scanner
```

Run
```bash
go test "github.com/DataDog/datadog-iac-scanner/e2e" -v -count=1 -tags dev
```
...
```go
// E2E-CLI-020 - scan with --exclude-queries flag
...
```go
	// E2E-CLI-006 - generate-id should exhibit
...
Files created by the scanner during testing should always be created in the 'outputs' folder
...
In contrast to what is expected for results and payload, log files don't have a file to compare, instead the test suite validates logs like it validates CLI's output.
...
```go
// E2E-CLI-038 - scan command with --log-path
	...
	args: args{
		...
		expectedLog: LogValidation{
			...
			validationFunc: func(logText string) bool {
				match1, _ := regexp.MatchString(`Files scanned: \d+`, logText)
				match2, _ := regexp.MatchString(`Queries loaded: \d+`, logText)
...
func Test_E2E_CLI(t *testing.T) {
	scannerPath := GetScannerLocalBin("")
...
<b>GetScannerLocalBin:</b> This function gets the path of the scanner tool.

<b>runCommand:</b> This function is intended to run the scanner with the input arguments sent by the test
...
<b>checkLine & checkJSONLog:</b> These functions are used to check the scanner output generated in the CLI and compare it with the <b>expectedOutput</b> file.
