...
// 1. Terraform plan JSON files are scanned
...
func Test_E2ETerraformPlanFlag(t *testing.T) {
	...
	t.Run("tfplan scanned", func(t *testing.T) {
		params, ctx := scan.GetDefaultParameters(context.Background(), "")
		...
		require.Greater(t, metadata.Stats.Files, 0, "Terraform plan JSON should be scanned")
	})

	t.Run("cloudformation json not scanned", func(t *testing.T) {
		params, ctx := scan.GetDefaultParameters(context.Background(), "")
		...
		// only processes Terraform JSON files
