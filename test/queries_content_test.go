...
import (
	...
	"github.com/DataDog/datadog-iac-scanner/pkg/engine/mock"
...
var (
	...
	requiredQueryMetadataProperties = map[string]func(tb testing.TB, value any, metadata map[string]any,
		...
		"id": func(tb testing.TB, value any, metadata map[string]any, metadataPath string) {
			...
			queryDir := filepath.Base(filepath.Dir(metadataPath))
			require.Equal(tb, strings.TrimSpace(idValue), utils.ToID(platform, cloudProvider, queryDir), "invalid UUID in query metadata file %s", metadataPath)
		},
...
func testQueryHasGoodReturnParams(t *testing.T, entry queryEntry) { //nolint
	...
	queriesSource.EXPECT().GetQueryLibrary(gomock.Any(), "common").
		DoAndReturn(func(ctx context.Context, platform string) (source.RegoLibraries, error) {
			q, err := readLibrary(platform)
			require.NoError(t, err)
			return q, nil
		})
	queriesSource.EXPECT().GetQueryLibrary(gomock.Any(), entry.platform).
		DoAndReturn(func(ctx context.Context, platform string) (source.RegoLibraries, error) {
			q, err := readLibrary(platform)
			require.NoError(t, err)
			return q, nil
	...
	inspector, err := engine.NewInspector(
		ctx,
		queriesSource,
		func(ctx context.Context, qCtx *engine.QueryContext, trk engine.Tracker, v interface{}, detector *detector.DetectLine, useOldSeverities bool, kicsComputeNewSimID bool, queryDuration time.Duration) (*model.Vulnerability, error) {
